...
def get_node_content(documents: dict, doc_id: str, node_ids: str) -> str:
    """
    Retrieve content for specific nodes in a document.
    ...
    try:
        parsed_node_ids = _parse_node_ids(node_ids)
    except (ValueError, AttributeError) as e:
        return json.dumps(
            {
                "error": (
                    f"Invalid node_ids format: {node_ids!r}. "
                    f'Use "12", "12,15", or "12.1,12.2". '
                    f"Error: {e}"
                )
            },
            ensure_ascii=False,
    ...
    try:
        content = _get_node_content(doc_info, parsed_node_ids)
    except Exception as e:
        return json.dumps(
            {"error": f"Failed to retrieve node content: {e}"},
            ensure_ascii=False,
        )
...
def _parse_node_ids(node_ids: str) -> list[str]:
    """
    ...
    if not node_ids or not isinstance(node_ids, str):
        raise ValueError("node_ids must be a non-empty string")
    ...
    if not parsed:
        raise ValueError("No valid node_ids found")

    return parsed


def _get_node_content(doc_info: dict, node_ids: list[str]) -> list[dict]:
    """
    Retrieve node content from the document structure.

    Assumes:
        doc_info["nodes"] is a dict keyed by node_id
    ...
    for node_id in node_ids:
        ...
        for each_node in nodes:
            if each_node["node_id"] == node_id:
                results.append(
                    {
                        "node_id": node_id,
                        "summary": each_node.get("summary", ""),
                        "text": each_node.get("text", ""),
                    }
                )
